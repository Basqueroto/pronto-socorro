...
import type { PatientInsert, PatientUpdate, ArchivedPatientInsert } from "./supabase"
  ...
  return {
    ...
    reevaluationRequest: dbPatient.reevaluation_requested
      ? {
          ...
          timestamp: new Date(dbPatient.reevaluation_timestamp),
...
export async function getAllPatients(): Promise<Patient[]> {
  ...
  return Array.isArray(data) ? data.map(dbToPatient) : []
}
...
export async function requestReevaluation(patientId: string, reason: string): Promise<Patient | undefined> {
  try {
    ...
    if (patientError || !patientData || patientData.length === 0) {
      console.error("Erro ao buscar paciente para reavaliação:", patientError)
      return undefined
    }

    // Atualizar o paciente com a solicitação de reavaliação
    ...
    if (updateError) {
      console.error("Erro ao atualizar solicitação de reavaliação:", updateError)
      return undefined
    }
    ...
    if (fetchError || !updatedData || updatedData.length === 0) {
      console.error("Erro ao buscar paciente atualizado:", fetchError)
      return undefined
    }

    return dbToPatient(updatedData[0])
  } catch (error) {
...
export async function markReevaluationAsSeen(patientId: string): Promise<Patient | undefined> {
  try {
    ...
    if (updateError) {
      console.error("Erro ao marcar reavaliação como vista:", updateError)
      return undefined
    }
...
export async function updatePatientStep(
  ...
  try {
    ...
    if (updateError) {
      console.error("Erro ao atualizar etapa do paciente:", updateError)
      return undefined
    }
